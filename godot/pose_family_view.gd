...
# Value-only source pose presentation. No Godot skeleton/animation/physics
# state feeds the native owner. Materials are deliberately unlit diagnostics.
...
func configure(packet: Dictionary) -> void:
    assert(packet.ok and not packet.presentation_feedback and packet.exclusive_parser)
    assert(_nodes.is_empty())
    _model = packet.model
    _topology = packet.meshes.duplicate(true)
    var shader := Shader.new()
    shader.code = POSE_SHADER
...
func present(packet: Dictionary) -> void:
    assert(packet.ok and packet.model == _model and packet.meshes.size() == _nodes.size())
