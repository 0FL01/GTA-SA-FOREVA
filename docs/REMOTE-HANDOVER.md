...
**Latest follow-up:** native `e7d7af08` pushed; root companion contains this checkpoint (base `efc036e`). Step planner (`c3bea960`) passes44 physical checks/full-source ASan/UBSan. Prepared ped flag corrected to `CollisionProcessed`/0x2, not `HasContacted`/0x8, before friction integration; preparation16/model26 plus query/preparation sanitizers pass. Contact315 and coherent build pass; extension unchanged from fresh delivery below. Logs `p3-ped-steps-*`, `p3-ped-model-*`. P3-A02 remains open for actual driver/friction/support/world route.
