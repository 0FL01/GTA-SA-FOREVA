...
Updated: 2026-09-12. Working directory: `/home/opencode/ai/mad-sa`, user `opencode`.
**Latest checkpoint:** P3-A06 source vehicle lifecycle at native `b2cc5cc9`; root companion is the commit containing this checkpoint, based on `32a8bd7`. The76-check normal Mode0 route consumes a genuine sole-worker generation-qualified model400 completion, crosses phased EnterCar701/CarDrive710/LeaveCar704 with source camera completion, drives the exact A05 Automobile against retained source COL, then detaches the player, destroys/releases ref1, evicts generation10, migrates the ped and respawns ref2 in generation11. Pool events are value-only and no longer retain released model/COL owners. Direct+ASan/UBSan, pool/queue, native33/0, read-only/no-EXE and fresh package Vulkan routes pass. Extension/package `6d00c0bef914a5d805c0ab6189b3036f3ea13e1342a17715a02a3f08d8f0c1ca`; PID1 docker-init/zombies0. This verifies A06:22/68=32.4%, not readiness.
**Active P3-A07:** connect normal action input to authoritative required HUD state and basic SFX events for this exact source lifecycle. Do not promote `RealtimeGameplay`/diagnostic actor presentation. Complete door task bodies/world candidate discovery, dynamic vehicle/object response, broad world/vehicle families and full Godot gameplay hosting remain later atoms. All six axes and target reflight remain open.
**Integrated checkpoint:** P3-A01 ordinary on-foot/jump/interruption boundary verified at native `45f192a4` / root `6163cf1`.294 integrated,57 manager and107 real-bank checks pass, including all-relevant-source task/manager ASan/UBSan. Shared COL face-group metadata changes extension SHA256 to `d35fe301e59cd40a367ab74eea3bb60ca6f36cf6ef9ca3c28827393b89de0f48`; fresh clean Vulkan actor/region/async and native33/0 sweep pass. Special control branches remain explicit, not ordinary-run substitutes; synthetic contact observations are not physics. Count17/68, not game readiness. Evidence/commands in the [single current checkpoint](goals/2026-09-11-godot-full-port.md#current-checkpoint-and-evidence) and Godot README.
**ACTIVE, DIRECT AT USER REQUEST:** no subagents; no compress unless critical. Owned game is `Grand-Theft-Auto-San-Andreas/` here, mounted `/game:ro`; static retail RE mapped SetRealMoveAnim to0x627780, not upstream0x60A9C0. Runtime never reads EXE. The actor studio remains an approximation, not source gameplay; atom count is not game readiness.
**A02 boundaries:** owner requires complete/deduplicated source-ordered normal-sector input; actual initial whole-world publication remains Unsupported where order/empty-model/LOD authority is unknown. Moving supports, head/height/fall task effects, blocked-position pieces and cross-sector pool traversal remain later work, not successful defaults. Controlled geometry proves curb/wall/pair orchestration; only the separate support witness is actual3991 geometry. Logs `artifacts/build-runs/p3-ped-world-*`; static RE constants/helpers in `p3-a02-ped-support-*`. Protected unrelated untracked files remain untouched.
...
P0 delivery: `artifacts/godot/sa-legacy-look-lab-p0-linux-x86_64.tar.gz` (asset-free, pinned runtime included), details in GODOT-P0-03. P1-A01 adds TXD-qualified identity in native `ddc9863d`; package `artifacts/godot/package/` passed real identity/upload/readback and retained-world gates. P1-A02 extends prepared catalogs only: metadata/binary-storage/census gates and ground regression pass, product build is unchanged. Source NaN candidates still reject with provenance while the last world stays live. Target reflight remains open.
...
Read first: `AGENTS.md`, the current full-port goal checkpoint and `docs/PORT-READINESS.md`. Consult only specifically linked evidence when needed; old journals/native goals are historical, not a prerequisite to reconstruct this checkpoint.

## Repository and current work

- **Latest task:** user resumed full-goal execution and then required DIRECT work without orchestration. Preserve unrelated work and the single roadmap; target reflight and F1/F2/R6 remain open.
