...
**Latest checkpoint:** native `8faf5bee` remains pushed; root companion contains this checkpoint. The staged Godot package is regenerated from an allowlist and passes `godot-package-audit-ok files=26 assets=0 symlinks=0 ... exe=0 wine=0 licenses=godot,godot-cpp,librw external-game-dir=required`; it launches against `/game:ro` from its own tree.
**Current atom:** P9-A01 finish-line audit;60/68 verified (88.2%), not readiness. P4 and P9-A05 are complete. P7-A06 target audio, P7-A10 startup movie codec/license path, P0 target reflight and the remaining P9 release gates remain open.
