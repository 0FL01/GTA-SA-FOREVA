...
**Latest checkpoint:** native `718bd57b` is pushed; root companion contains this checkpoint. Actual owned `Logo.mpg` and `GTAtitles.mpg` decode as MPEG1+MP2 through dynamic system FFmpeg, and the source `loadsc0` splash decodes through the sole texture path. Native sweep is now34/0. `docs/FFMPEG-RUNTIME.md` records the LGPL/system-dependency boundary; no movie or codec library enters the asset-free package.
**BLOCKED:**61/68 verified (89.7%), not readiness. All independent work is complete. P0-A04 and P7-A06 require the unavailable Fedora44/Wayland/Radeon780M session with real audio; P9-A02/A03 require that target, and P9-A01/A04/A06 depend on those results. Run `tools/target-closure.sh` on the user target and preserve its artifact directory; do not substitute server llvmpipe/Dummy evidence.
