...
**RESUMED:** user replaced `@general` with `@cheap-worker` default / `@costly-worker` for ambiguous decisions; no fixed worker count. Protocol root5c9881e and P1-A03 native0628fce4 are pushed. A03's unique source LinkLods chain is CPU-verified, not full runtime LOD. P1-A04 actual render/COL generation-boundary research is current; its first read-only call was interrupted and the user requested continuation. See goal/evidence; historical pause is superseded.
