...
**Latest checkpoint:** P4-A02 at native `86f7e38e`; root companion is the commit containing this checkpoint, based on `c19ae72`. The sole Session owns stack8, tag0 main/streamed child parameters, mission/streamed BaseIP, generation-qualified external payloads and source byte user counts. Real main metadata↔`script.img` name mapping verifies all79 definitions; exact requested bytes stay private. The106-check normalized route repeats31 commits with pass shape18/9/1/1/1/1 and mission→streamed→child order, independent waits and zero-user eviction/reload. Scheduling+Frame ASan/UBSan, frame104/session5467, strict corpus fingerprints, native smoke/sweep33/0 and read-only/no-EXE trace pass. Extension/package remains `6d00c0bef914a5d805c0ab6189b3036f3ea13e1342a17715a02a3f08d8f0c1ca`. P4-A02 verified:25/68=36.8%, not readiness.
**Active P4-A03:** define stable asynchronous script-service prepare/pending/commit/cancel semantics with one request identity, exactly-once commit and no fake readiness. Generated scheduling bytecode does not claim every streamed body or entity-attached brain activation. `0814` owner/runtime, genuine boot/save and all six finish axes remain open.
