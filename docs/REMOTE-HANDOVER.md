...
**Latest follow-up:** native `34a1a6a5` pushed; root companion contains this checkpoint (base `5798307`). Physical61/full-source ASan/UBSan now includes source friction accumulation/translation consumption, alongside step planning and forces. Prepared flag is `CollisionProcessed`/0x2, not `HasContacted`/0x8. Preparation16/model26 and contact315 regressions pass; extension unchanged from fresh delivery below. Latest logs `p3-ped-friction-*`, `p3-ped-model-*`. P3-A02 remains open for material binding, actual driver/rollback/support and the loaded-world route; friction coefficients are still caller-supplied.
...
**A02 in progress:** physical31/contact315/ped-preparation16/model-pair26 direct/sanitized checks pass; real contact fixture covers122 COL triangles/four groups, and prepared support line hits the grouped model, not whole-world coverage. Retail CPed constructor0x6044A0/vtable0x8B6FBC/ProcessEntityCollision0x5FE210 mapped. Latest logs `artifacts/build-runs/p3-contact-metadata-*` and `p3-ped-model-*`; core20 TUs, same fresh extension hash. Next ped support/physical response and complete loaded-world candidate ownership; see current goal. Protected unrelated untracked files remain untouched.
