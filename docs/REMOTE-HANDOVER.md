...
**Latest follow-up:** native `30bb9b7a` pushed; root companion contains this checkpoint (base `69a9893`). Source adhesion/material binding now has429 direct/sanitized checks over real179 materials plus synthetic parser/retention/friction cases. Canonical source names are shared, coefficients remain external/read-only. Physical61/model26/contact315/frame104, native build/smoke, IO/link closure and Win32 lookup syntax pass; extension unchanged from fresh delivery below. Latest logs `p3-surfaces-*`; core22 TUs. P3-A02 remains open for actual driver/rollback/support, sector ownership and the loaded-world route; no source gameplay promotion.
...
**A02 contact baseline:** physical force/contact31, ped-preparation16 and model-pair26 direct/sanitized checks pass; contact315 covers122 COL triangles/four groups, and prepared support line hits the grouped model, not whole-world coverage. Retail CPed constructor0x6044A0/vtable0x8B6FBC/ProcessEntityCollision0x5FE210 mapped. Logs `artifacts/build-runs/p3-contact-metadata-*` and `p3-ped-model-*` preserve this earlier20-TU checkpoint. Prepared flag is `CollisionProcessed`/0x2, not `HasContacted`/0x8. Next ped support/physical response and complete loaded-world candidate ownership; see current goal. Protected unrelated untracked files remain untouched.
