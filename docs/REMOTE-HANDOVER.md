...
**Latest checkpoint:** P4-A04 at native `988ff5e9`; root companion is the commit containing this checkpoint, based on `eab88fc`. A canonical v1 little-endian envelope preserves one quiescent `NativeScriptSession` graph through writer/reader `exec` processes while assets remain externally and independently loaded. The6863-byte fixture owns main/mission/streamed stacks, globals/policies and live streamed users; restore retains the new process session epoch, re-encodes identically and matches continuation `3,3,3,1`.111 reader checks plus writer rejections, ASan/UBSan, session/scheduler/transaction/frame/startup regressions, native smoke/sweep33/0 pass. Extension/package remains `6d00c0bef914a5d805c0ab6189b3036f3ea13e1342a17715a02a3f08d8f0c1ca`. P4-A04 verified:27/68=39.7%, not readiness.
**Active P4-A05:** implement the exact real owners/services demanded by unchanged startup, starting with `0814` registration. Registration, runtime update/reward/reset and persistence coverage remain separate; do not use the portable script-only envelope to claim original PC save compatibility or a complete gameplay save. Genuine boot and all six finish axes remain open.
