...
**RESUMED:** user replaced `@general` with `@cheap-worker` default / `@costly-worker` for ambiguous decisions; no fixed worker count. Protocol root5c9881e and P1-A02 native2d121583 are pushed. P1-A03 is current: one source LinkLods chain, not full runtime LOD. See current goal/evidence; historical pause is superseded.
