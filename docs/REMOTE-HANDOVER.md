...
**ACTIVE:** cheap default/costly ambiguity, no fixed count. P2-A04 world-adoption seam verified; native `2457d32b` pushed, root companion is the commit containing this checkpoint, based on `98e839a`. Core keeps explicit `04E4` collision frontier (`ip=56022 next=56034`), exit0 fixture success not boot; existing native Host main53/mission1234/0814 frontier stays separate — neither is full boot. Extension SHA256 NEW `0e2e1115ba6f2838daf29dc46dd4882f7b53118447ba816fc0a4931090d9d598` (rehome layout; ONLY `NativeSourcePad.cpp.o` into `sa_legacy`, sole `sa_legacy_library_init` export, not world-hosted yet); `sa_core` now 9 exact TUs with 79-check world probe. Current P2-A05 is in_progress SEMANTIC RESEARCH ONLY (no implementation). See goal checkpoint/history and journal GODOT-P2-15; historical pause is superseded.
...
Read first: `AGENTS.md`, `docs/PORT-READINESS.md`, `docs/LIVE-TEST-JOURNAL.md`, then the current checkpoint of `docs/goals/2026-09-11-godot-full-port.md`. Old native goal is historical regression evidence.
