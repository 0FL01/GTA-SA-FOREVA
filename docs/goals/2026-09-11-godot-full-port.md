...
Status: ACTIVE
Execution: ACTIVE — @cheap-worker default / @costly-worker for ambiguous decisions; P2-A04 verified, P2-A05 in_progress SEMANTIC RESEARCH ONLY (no implementation).
...
| P2-A05 | in_progress | Add committed snapshots, ordered presentation events and SCM scheduling seams without leaking Godot/RW pointers. | Boundary test serializes/inspects one tick and rejects presentation-owned pointers or mutation. |
...
- **Current checkpoint:** P2-A04 verified, closed world-adoption seam; P2-A05 selected `in_progress` SEMANTIC RESEARCH ONLY (no implementation). Native `2457d32b` pushed; root companion is the commit containing this checkpoint, based on `98e839a`. Detail in history P2-A04 entry below and journal GODOT-P2-15, not duplicated here.
