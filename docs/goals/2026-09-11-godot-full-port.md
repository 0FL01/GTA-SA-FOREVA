...
Status: ACTIVE
Execution: ACTIVE — @cheap-worker default / @costly-worker for ambiguous decisions; P1-A06 verified, P1-A07 semantic research selected.
...
| P1-A07 | in_progress | Expand catalog-backed residency from the authored chain to the approved exterior itinerary and one interior, removing the lab's final `256`-instance visibility cap. Research selected, no implementation yet. | Revisit itinerary accounts for all expected placements and has bounded memory after repeated region/interior transitions. |
...
- **Current checkpoint:** P1-A06 verified; root companion containing this checkpoint, based on `4ea8d9e`; native `95713252` unchanged. See `### P1-A06 budgeted publication/retirement` history and journal `GODOT-P1-10` for all gate evidence, hashes, fixes and quota semantics (budget1 fixture quota1/cap16; production default quota64/cap256).
- P1-A07 is `in_progress` for semantic research only, no implementation yet. All six full-port axes plus P0 target reflight remain open; next is catalog-backed exterior/interior with no final 256 cap, not new physics/full-LOD.
- Remaining limit: final normal 40s package route exits 0 with `ready_count=0/submit=7/cancel=7`, Grove retained — slow-llvmpipe continuous movement outran prepared uploads. Safe lifecycle only, not load progress. Protected user configuration/TGA/caches untouched.
