...
Status: ACTIVE
Execution: ACTIVE — @cheap-worker default / @costly-worker for ambiguous decisions; P1-A03 verified, P1-A04 boundary research selected.
...
| P1-A04 | in_progress | Couple that chain's COL relationship and generation to world publication. | A generation switch publishes matching render/collision lineage or publishes neither. |
...
- **Current checkpoint:** P1-A03 native `0628fce4` is verified/pushed, with evidence below. P1-A04 now researches the smallest actual render/COL generation publication boundary for this chain using existing owners; the first read-only costly call was interrupted by connectivity and is retried after user resume. No A04 implementation or runtime claim yet. All other catalog runtime fields remain Unknown, A05 async is pending, and all six full-port axes/target reflight remain unresolved. Protected user configuration/TGA/caches are untouched.
