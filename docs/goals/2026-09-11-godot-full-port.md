...
Status: ACTIVE
Execution: ACTIVE — @cheap-worker default / @costly-worker for ambiguous decisions; P1-A04 verified, P1-A05 parser-worker ownership selected.
...
| P1-A05 | in_progress | Adapt the sole-owner parser worker/mailbox for asynchronous raw parsing with request and generation identity. | Queue/parse cancellation test proves stale results cannot cross generations. |
...
- **Current checkpoint:** P1-A04 native `95713252` plus root containing commit are verified. Paired render/COL owners survive rejection/teardown; the Vulkan-only C++ runtime collision is fixed. P1-A05 sole-owner worker/mailbox research is selected, not implemented. Global catalog runtime fields remain Unknown; A06 budgets, all six full-port axes and target reflight remain unresolved. Protected user configuration/TGA/caches are untouched.
